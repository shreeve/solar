 ...
 * Clean implementation influenced by Jison, implemented in pure JavaScript.
 *
 * Performance: ~58ms on Bun, ~180ms on Node.js (~215× faster than Jison)
 *
 * Install and use:
...
// =============================================================================
// Generator - Main SLR(1) parser generator
// =============================================================================
...
class Generator {
  ...
  _insertStateWithItems(symbol, items, itemSet, states, stateMap) {
    ...
    const kernel = items.map(item => [item.rule.id, item.dot + 1]);
  ...
  _computeFirstOfSequence(symbols) {
    ...
    for (const symbol of symbols) {
      if (this.types[symbol]) this.types[symbol].firsts.forEach(s => firsts.add(s));
      else firsts.add(symbol);
  ...
  buildParseTable(itemSets = this.states) {
    ...
    for (let k = 0; k < itemSets.length; k++) {
      ...
      for (const [stackSymbol, gotoState] of itemSet.transitions) {
        ...
        if (types[stackSymbol]) state[this.symbolIds[stackSymbol]] = gotoState;
        else                    state[this.symbolIds[stackSymbol]] = [SHIFT, gotoState];
      }
        ...
        for (const stackSymbol of item.lookaheads) {
          ...
          if (action) {
            ...
            const solution = this._resolveConflict(
              item.rule, op, [REDUCE, item.rule.id], which
            );

            if (solution.bydefault) this.conflicts++;
            else action = solution.action;
          } else {
  ...
  _resolveConflict(rule, op, reduce, shift) {
    const solution = { rule, operator: op, r: reduce, s: shift };
    ...
    if (rule.precedence === 0 || !op) {
      ...
      switch (op.assoc) {
        case "right":    solution.action = shift   ; break;
        case "left":     solution.action = reduce  ; break;
        case "nonassoc": solution.action = NONASSOC; break;
        default:         solution.action = shift   ; break;
      }
    } else               solution.action = reduce;
  ...
  generate() {
    const parserInstance = this._generateModuleCore();
...
`;
  ...
  _generateTableCode(stateTable) {
    return JSON.stringify(stateTable, null, 0).replace(/"([0-9]+)"(?=:)/g, "$1");
  }
  ...
  parse(input) {
    ...
    const stk = [0], val = [null], loc = [],{ parseTable } = this, [TERROR, EOF] = [2, 1];
    ...
    while (true) {
      ...
      switch (action[0]) {
        ...
        case 2: // reduce
          ...
          r = performActionFn.apply(yyval, [
            yytext, yyleng, yylineno, sharedState.yy, action[1], val, loc
          ]);
          if (r != null) yyval.$ = r;
          if (len) {
            stk.length -= len * 2;
...
export function Parser(grammar, options) {
...
const isRunAsScript = process.argv[1] && (
...
`);
    ...
    while (i < process.argv.length - 2) {
      ...
      switch (arg) {
        ...
        case '-s': case '--sexpr':   options.sexpr   = true; break;
        case '-o': case '--output':  options.output  = process.argv[++i + 2]; break;
        default: if (!arg.startsWith('-')) grammarFile = arg;
      }
      i++;
    }
    ...
    if (!grammarFile   ) { showHelp()   ; process.exit(0); }
      ...
      if (options.sexpr) {
        ...
        if (grammar.grammar) {
          ...
          for (const [name, productions] of Object.entries(grammar.grammar)) {
            parts.push(`    (${name}`);
            for (const [pattern, action = 1, opts] of productions) {
